...
<div style="max-height: 560px; overflow-y: auto; border: 1px solid #ddd; padding: 10px;">
    <table class="table table-hover align-middle mb-0">
        ...
        <tbody>
            ...
            @php($statusFluxo = $processo->statusApresentacaoDefinicao())
            <tr>
                <td>
                    <div class="fw-semibold">{{ $processo->titulo }}</div>
                    <div class="text-muted small">{{ $processo->numero_processo ?: 'Número pendente' }} • Edital
                        {{ $processo->numero_edital }}</div>
                </td>
                <td>{{ $processo->empresa?->nome_razao_social }}</td>
                <td>{{ $processo->tipo_processo === 'concurso_publico' ? 'Concurso Público' : 'Processo Seletivo' }}
                </td>
                <td>
                    <span class="badge {{ $statusFluxo['badge_class'] }}">{{ $statusFluxo['titulo'] }}</span>
                </td>
                <td>
                    <div class="small">{{ $processo->data_inicio_inscricoes?->format('d/m/Y H:i') ?: 'Não definido' }}
                    </div>
                    <div class="small text-muted">até
                ...
                <td>
                    ...
                    <a href="{{ route('sigeconcursos.processos.inscricoes', $processo->id_processo) }}"
                        class="btn btn-sm btn-outline-secondary">Inscrições</a>
                    <a href="{{ route('sigeconcursos.processos.isencoes', $processo->id_processo) }}"
                        class="btn btn-sm btn-outline-warning">Isenções</a>
            ...
            @empty
            <tr>
                <td colspan="6" class="text-center text-muted py-4">Nenhum processo encontrado com os filtros
                    informados.</td>
            </tr>
            @endforelse
        </tbody>
